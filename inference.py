...
def predict(model, test_loader):
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            if len(data) == 2:
                ...
                if config.mode == 'intersection_and_union_inference' or config.mode == 'intersection_inference ' or config.mode == 'both':
                    true_masks.append(ids1.cpu())  
                    true_masks2.append(ids2.cpu())
    ...
    if config.mode == 'intersection_inference' or config.mode == 'intersection_and_union_inference' or config.mode == 'both':
...
saved_model_name = 'One_segment_lossAnnotator_1_k=5_Model_smpUNet++_Augmentation_False_Modemulticlass_Optimizer_Adam_Scheduler_CosineAnnealingLR_Epochs_200_Batch_Size_6_Start_lr_0.001_Loss_CrossEntropyLoss_Timestamp_2024-08-06-12-23_best_model_iou_multiclass'
